...
namespace Assignment.Core.Validators
{
    public class CreateProductDTOValidator : AbstractValidator<CreateProductDTO>
    {
        public CreateProductDTOValidator()
        {
            ...
            RuleFor(x => x.Name).Matches("[a-zA-Z0-9]$").WithMessage("Please Enter Valid Name");
            RuleFor(x => x.ExpiryDate).Must(ValidateDate).WithMessage("Please Enter Valid Expiry Date");
            RuleFor(x => x.Price).Matches("^(\\d{1,10}\\.\\d{2,2})($|\\.\\d{2,2}$)").WithMessage("Please Enter Valid Price");
        }
